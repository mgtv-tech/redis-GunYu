...
func Int64(reply interface{}, err error) (int64, error) {
	...
	switch astyle := reply.(type) {
	case int64:
		return astyle, nil
	case []byte:
		n, err := strconv.ParseInt(string(astyle), 10, 64)
		return n, err
	case string:
		n, err := strconv.ParseInt((astyle), 10, 64)
		return n, err
	case nil:
		return 0, ErrNil
	case RedisError:
...
func StringIsOk(reply interface{}, err error) error {
	ret, err := String(reply, err)
	if err != nil {
		return err
	}
	if ret != ReplyOk {
		return fmt.Errorf("reply is not ok : %s", ret)
	}
	return nil
}
...
func convertAssign(d interface{}, s interface{}) (err error) {
	...
	case []byte:
		...
		case *string:
			*d = string(s)
		case *int:
			*d, err = strconv.Atoi(util.BytesToString(s))
		case *int64:
			*d, err = strconv.ParseInt(util.BytesToString(s), 10, 64)
		case *bool:
			*d, err = strconv.ParseBool(util.BytesToString(s))
		case *[]byte:
			*d = s
		case *interface{}:
			*d = s
