...
TMP_ROOT="${TMPDIR:-/tmp}/redisgunyu-nonbisync-cat2"
source "${ROOT}/tests/nonbisync/lib/test_env.sh"

SCENARIOS="${SCENARIOS:-sync,pipeline}"
SYNC_SRC_BASE="${SYNC_SRC_BASE:-31500}"
SYNC_DST_BASE="${SYNC_DST_BASE:-31600}"
SYNC_HTTP_PORT="${SYNC_HTTP_PORT:-31580}"
PIPE_SRC_BASE="${PIPE_SRC_BASE:-31700}"
PIPE_DST_BASE="${PIPE_DST_BASE:-31800}"
PIPE_HTTP_PORT="${PIPE_HTTP_PORT:-31780}"
TEST_PREFIX="${TEST_PREFIX:-nonbisync:cat2:$(date +%s)}"
SYNCER_PID=""
...
assert_expected_state() {
  ...
  expect_eq "$(redis_call cluster "${port}" get "$(key_name "${prefix}" "counter")")" "12" "$(key_name "${prefix}" "counter")"
  expect_eq "$(set_state cluster "${port}" "$(key_name "${prefix}" "set")")" "blue green red" "$(key_name "${prefix}" "set")"
  expect_eq "$(hash_state cluster "${port}" "$(key_name "${prefix}" "hash")")" "f1=v1|f2=v2" "$(key_name "${prefix}" "hash")"
  expect_eq "$(list_state cluster "${port}" "$(key_name "${prefix}" "list")")" "a b c d" "$(key_name "${prefix}" "list")"
...
run_scenario() {
  ...
  echo "[category2] scenario=${name} mode=${mode}"
